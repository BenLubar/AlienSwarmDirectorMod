...
ConVar  asw_test_new_alien_jump( "asw_test_new_alien_jump", "1", FCVAR_CHEAT );
...
bool CASW_Alien_Jumper::IsUnusableNode(int iNodeID, CAI_Hint *pHint)
{
	...
	if ( !asw_test_new_alien_jump.GetBool() )
		 return iBaseReturn;
...
bool CASW_Alien_Jumper::IsJumpLegal( const Vector &startPos, const Vector &apex, const Vector &endPos ) const
{
	...
	if ( HasSpawnFlags( SF_ANTLION_USE_GROUNDCHECKS ) && asw_test_new_alien_jump.GetBool() )
	{
		trace_t	tr;
