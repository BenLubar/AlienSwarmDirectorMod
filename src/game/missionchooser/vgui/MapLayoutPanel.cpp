...
void CMapLayoutPanel::AddRoom( const CRoomTemplate *pTemplate, int iTileX, int iTileY )
{
	...
	CRoom *pRoom = new CRoom( GetMapLayout(), g_pTileGenDialog->m_pCursorTemplate->m_iszLevelTheme, g_pTileGenDialog->m_pCursorTemplate->GetFullName(), iTileX, iTileY, RandomInt( 0, INT_MAX ) );
