...
void CASWHud3DMarineNames::PaintMarineLabel(int iMyMarineNum, C_ASW_Marine * RESTRICT pMarine, C_ASW_Marine_Resource * RESTRICT pMR, bool bLocal)
{
	...
	*/
	...
	if (!!debugoverlay->ScreenPosition(vMarinePos - Vector(0.0f, 10.0f, 0.0f), screenPos))
	{
		...
		if ( ::input->CAM_IsThirdPerson() && pLocal->GetASWControls() == 1 )
		{
...
#undef PAD
		...
		if (!bMarineIsKnockedOut && (bMarineOnScreen || asw_medic_under_marine_offscreen.GetBool()) && asw_medic_under_marine.GetFloat() > 0 && (pMarine->GetHealth() <= pMarine->GetMaxHealth() * asw_medic_under_marine.GetFloat() || (pMarine->m_flEmoteMedicStart > 0 && pMarine->m_flEmoteMedicStart + asw_medic_under_marine_recall_time.GetFloat() > gpGlobals->curtime)))
		{
		...
		if (!bMarineIsKnockedOut && (bMarineOnScreen || asw_magazine_under_marine_offscreen.GetBool()) && asw_magazine_under_marine.GetFloat() > 0 && (pMR->GetClipsPercentForHUD() <= asw_magazine_under_marine.GetFloat() || (pMarine->m_flEmoteAmmoStart > 0 && pMarine->m_flEmoteAmmoStart + asw_magazine_under_marine_recall_time.GetFloat() > gpGlobals->curtime)))
		{
			// the icons size, squared box
			const int iIconSize = 48;
